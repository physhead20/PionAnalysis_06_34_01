...
// Thu Aug 24 08:33:02 2017 by ROOT version 5.34/36
// from TTree anatree/analysis tree
// found on file: ../ROOTFILES/DDMC_RunIINegPickyKaon_anatree.root
//////////////////////////////////////////////////////////

#ifndef KaonDDMC_RunII_h
#define KaonDDMC_RunII_h
...
public :
   ...
   Int_t           t0;
   Int_t           nclus;
   Double_t        clustertwire[64];   //[nclus]
   Double_t        clusterttick[64];   //[nclus]
   Double_t        cluendwire[64];   //[nclus]
   Double_t        cluendtick[64];   //[nclus]
   Int_t           cluplane[64];   //[nclus]
   Int_t           ntracks_reco;
   ...
   Int_t           primarytrkkey;
   Int_t           nhits;
   Int_t           hit_plane[1070];   //[nhits]
   Int_t           hit_wire[1070];   //[nhits]
   Int_t           hit_channel[1070];   //[nhits]
   Double_t        hit_peakT[1070];   //[nhits]
   Double_t        hit_charge[1070];   //[nhits]
   Double_t        hit_ph[1070];   //[nhits]
   Double_t        hit_tstart[1070];   //[nhits]
   Double_t        hit_tend[1070];   //[nhits]
   Int_t           hit_trkid[1070];   //[nhits]
   Int_t           hit_trkkey[1070];   //[nhits]
   Int_t           hit_clukey[1070];   //[nhits]
   Int_t           hit_pk[1070];   //[nhits]
   Int_t           hit_t[1070];   //[nhits]
   Int_t           hit_ch[1070];   //[nhits]
   Int_t           hit_fwhh[1070];   //[nhits]
   Double_t        hit_rms[1070];   //[nhits]
   Double_t        hit_nelec[1070];   //[nhits]
   Double_t        hit_energy[1070];   //[nhits]
   Float_t         hit_dQds[1070];   //[nhits]
   Float_t         hit_dEds[1070];   //[nhits]
   Float_t         hit_ds[1070];   //[nhits]
   Float_t         hit_resrange[1070];   //[nhits]
   Float_t         hit_x[1070];   //[nhits]
   Float_t         hit_y[1070];   //[nhits]
   Float_t         hit_z[1070];   //[nhits]
   Int_t           nwctrks;
   ...
   Int_t           maxTrackIDE;
   Double_t        IDEEnergy[3259];   //[maxTrackIDE]
   Double_t        IDEPos[3259][3];   //[maxTrackIDE]
   Int_t           no_primaries;
   ...
   KaonDDMC_RunII(TTree *tree=0);
   virtual ~KaonDDMC_RunII();
...
#ifdef KaonDDMC_RunII_cxx
KaonDDMC_RunII::KaonDDMC_RunII(TTree *tree) : fChain(0) 
{
// if parameter tree is not specified (or zero), connect the file
// used to generate this class and read the Tree.
   if (tree == 0) {
      TFile *f = (TFile*)gROOT->GetListOfFiles()->FindObject("../ROOTFILES/DDMC_RunIINegPickyKaon_anatree.root");
      if (!f || !f->IsOpen()) {
         f = new TFile("../ROOTFILES/DDMC_RunIINegPickyKaon_anatree.root");
      }
      TDirectory * dir = (TDirectory*)f->Get("../ROOTFILES/DDMC_RunIINegPickyKaon_anatree.root:/anatree");
...
KaonDDMC_RunII::~KaonDDMC_RunII()
{
   if (!fChain) return;
   delete fChain->GetCurrentFile();
}

Int_t KaonDDMC_RunII::GetEntry(Long64_t entry)
{
// Read contents of entry.
   if (!fChain) return 0;
   return fChain->GetEntry(entry);
}
Long64_t KaonDDMC_RunII::LoadTree(Long64_t entry)
{
...
void KaonDDMC_RunII::Init(TTree *tree)
{
...
void KaonDDMC_RunII::Show(Long64_t entry)
{
...
Int_t KaonDDMC_RunII::Cut(Long64_t entry)
{
...
#endif // #ifdef KaonDDMC_RunII_cxx
